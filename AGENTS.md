...
> **⚠️ Advertencia general sobre los SDKs (2026-07-02):** este repo es un fork de `ramiidv/arca-server` y depende de los SDKs `@ramiidv/arca-*` y `arca-cert` del mismo autor, que ya demostraron tener **varios bugs reales** (ver entradas de abajo: `iva`/`exento` descartado en silencio, header `SOAPAction` nunca enviado, SOAP faults tragados, `postSoap()` ignorando su parámetro, tickets WSAA cacheados solo en memoria, `.errors` de zod 3 en código que corre zod 4). Regla práctica: **ante cualquier comportamiento raro contra AFIP, sospechar primero del SDK** — leer su código en `node_modules`, reproducir el request a mano como oráculo (un TRA firmado con openssl + curl contra WSAA destrabó todo una vez), y si es un bug, parchearlo local con `patch-package` (patrón ya establecido en `backend/patches/`, se aplican solos en el postinstall). No asumir que "el SDK lo maneja".
...
- **2026-07-02**: Tres bugs de SDK encontrados y **parcheados localmente con `patch-package`** (`backend/patches/`, se aplican solos en el `postinstall`) al conectar con AFIP homologación real:
  1. `@ramiidv/arca-common` (`soap-client.js`): `if (opts?.soapAction)` — string vacío es falsy, así que el header `SOAPAction: ""` que WSAA exige **nunca se mandaba** → todo `loginCms` moría con HTTP 500 (`Client.NoSOAPAction`). Parcheado a `!== undefined`.
  2. `@ramiidv/arca-common` (mismo archivo): ante HTTP ≠200 tiraba `HTTP 500: Internal Server Error` **sin parsear el SOAP fault del body**, escondiendo el motivo real. Parcheado: el mensaje ahora incluye el faultstring.
  3. `arca-cert` (`http.js`): `postSoap()` recibía el parámetro `soapAction` y **lo ignoraba** (nunca lo agregaba al request). Mismo síntoma que (1) en el botón "Verificar con WSAA". Parcheado en las variantes ESM y CJS.
  Reportables upstream a los repos de ramiidv; mientras tanto los patches viven acá.
- **2026-07-02**: **Footgun de WSAA en desarrollo**: AFIP rechaza pedir un ticket de acceso (TA) nuevo mientras el anterior siga vigente (~12h) — `"El CEE ya posee un TA valido"` — y el SDK cachea los TA **solo en memoria**, así que un reinicio del backend después de un login exitoso puede dejar `wsfe` bloqueado horas. Mitigación en `arca.service.ts` (`seedAccessTickets`): si existe `backend/certs/ta-wsfe.json` (no versionado, shape `{token, sign, expirationTime}`), se siembra el cache del SDK al bootear. Si te quedás bloqueado sin ese archivo, se puede obtener un TA a mano firmando un TRA con openssl contra `https://wsaahomo.afip.gov.ar/ws/services/LoginCms` y guardarlo ahí. El fix de fondo (persistencia de TA a disco) corresponde al SDK.
- **2026-07-02**: Setup de certificados AFIP homologación **reales** vía WSASS manual (el flujo automático de la app requiere login al portal de homologación `authhomo-ext.afip.gob.ar`, que puede tener credenciales distintas a las de producción): generar clave+CSR con `openssl req -new -newkey rsa:2048 -nodes -keyout key.key -out pedido.csr -subj "/C=AR/O=<nombre>/CN=<alias>/serialNumber=CUIT <cuit>"`, pegar el CSR en WSASS → "Crear DN y certificado" (el CN debe coincidir con el nombre simbólico del DN; si tira `Error creando computador`, probar con otro alias), guardar el PEM devuelto como `certs/cert.crt`, y **autorizar el DN al servicio `wsfe`** en WSASS. Reiniciar el backend a mano (nodemon no vigila `.crt`/`.key`). Frontend quedó en 0 vulnerabilidades (se actualizaron `vite` 8.0→8.1, `react-router` 7.15+, `form-data`, `postcss`). Backend quedó con 4 dev-only que **no conviene forzar**: 3 son la cadena `prisma` (CLI) → `@prisma/dev` → `@hono/node-server`, cuyo "fix" es downgradear Prisma a 6.x (breaking, dirección equivocada); la otra es `esbuild@0.27.4` pineado por `tsx` y `vite`, con una vulnerabilidad del dev server propio de esbuild que ni tsx ni vitest usan. Revisitar cuando Prisma/tsx/vite publiquen updates.
...
- **2026-07-02**: AFIP exige la **condición de IVA del receptor** (`condicionIva` → `CondicionIVAReceptorId`) en todo comprobante con receptor identificado (`docTipo ≠ 99`) desde abril 2026 — el SDK lo valida y rechaza antes de llegar a AFIP. El backend la resuelve en `resolveCondicionIva()` (`invoice.service.ts`): body explícito → `ivaCondition` del cliente asociado → error 400. Ojo al probar: AFIP también rechaza que el receptor tenga el mismo CUIT que el emisor (error 10069), así que el cliente de prueba debe tener un documento distinto al del `.env`.
